...
class ModelPredictiveBuffer(object):
    """Buffer for storing and managing data collected during training.
    ...
    def __init__(self, config, obs_space, share_obs_space, act_space, num_agents):
        self.size = config["mpc_buffer_size"]
        self.episode_length = config["buffer_batch"]
        ...
        if type(share_obs_shape[-1]) == list:
            share_obs_shape = share_obs_shape[:1] # del
        self.share_obs = torch.zeros(self.size + 1, self.n_rollout_threads, self.num_agents, *share_obs_shape, device=self.device)
        self.obs = torch.zeros(self.size + 1, self.n_rollout_threads, self.num_agents, *obs_shape, device=self.device)
        ...
        act_shape = get_shape_from_act_space(act_space)
        self.share_obs_shape, self.obs_shape, self.act_shape = share_obs_shape[0], obs_shape[0], act_shape
        self.actions = torch.zeros(self.size, self.n_rollout_threads, self.num_agents, act_shape, device=self.device)
        self.action_log_probs = torch.zeros(self.size, self.n_rollout_threads, act_shape, device=self.device)
        self.rewards = torch.zeros(self.size, self.n_rollout_threads, self.num_agents, 1, device=self.device)
        
        self.costs = torch.zeros_like(self.rewards)

        self.masks = torch.ones(self.size + 1, self.n_rollout_threads, self.num_agents, 1, device=self.device)
        self.bad_masks = torch.ones_like(self.masks)
        self.active_masks = torch.ones_like(self.masks)
        self.additional_information = torch.zeros(self.size+1, self.n_rollout_threads, 4, device=self.device)
        self.factor = torch.ones(self.size, self.n_rollout_threads, 1, device=self.device)
    ...
    def insert(self, share_obs, obs, actions, masks, additional_information):
        """
        Inserts data from a single time step into the replay buffer.
        ...
        if self.index >= self.size:
            self.index = 0
        self.share_obs[self.index+1].copy_(share_obs.view(self.n_rollout_threads, self.num_agents, self.share_obs_shape))
        self.obs[self.index+1].copy_(obs.view(self.n_rollout_threads, self.num_agents, self.obs_shape))
        self.actions[self.index].copy_(actions.view(self.n_rollout_threads, self.num_agents, self.act_shape))
        self.additional_information[self.index+1].copy_(additional_information.view(self.n_rollout_threads, 4))


        self.masks[self.index+1].copy_(masks.view(self.n_rollout_threads, self.num_agents, 1))

...
class Runner:
    ...
    def __init__(self,
        ...
        self.logger = EpochLogger(
            log_dir = '/local/scratch/g/pu36/Safe-Policy-Optimization/logs',
            seed = str(config["seed"]),
        )
        self.save_dir = str('/local/scratch/g/pu36/Safe-Policy-Optimization/logs'+'/models_seed{}'.format(self.config["seed"]))
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.config["episode_length"]):
                ...
                self.insert(data)
            wandb.log({'MARL cost': torch.sum(train_episode_costs), 'MARL reward': torch.sum(train_episode_rewards)})
            self.compute()
            self.train()
    ...
    def restore(self):
        ...
        model_predictor_dict = torch.load(str(self.model_dir) + '/predictor_model.pt')
        self.predicit_model.load_state_dict(model_predictor_dict)
    ...
    def eval(self, eval_episodes=1):
        for episode in range(self.config['mpc_episodes']):
            eval_episode = 0
            ...
            for step in range(self.config["episode_length"]):
                ...
                episode_reward += reward_env.sum()
                episode_cost += cost_env.sum()
                ...
                if step>=self.config["episode_length"]:
                    print('rl cost is:', torch.sum(episode_cost))
                    print('rl reward is:', torch.sum(episode_reward))
                    print('rl reward is:', torch.sum(episode_reward))
                for eval_i in range(self.config["n_eval_rollout_threads"]):
                    if eval_dones_env[eval_i] or step>=self.config["episode_length"]:
                        eval_episode += 1
        ...
        for mpc_episode in range(self.config['mpc_episodes']):
            ...
            for step in range(self.config["episode_length"]):
                # MPC actions
                action, cost = self.mpc_optimization(input, prediction_horizon=3)
                action_tensor = torch.tensor(action).view(self.num_agents, self.config["n_rollout_threads"], self.action_size).to(self.config['device'])
                action = [action_tensor[i] for i in range(action_tensor.shape[0])]
                obs, share_obs, rewards, costs, dones, infos, _ = self.envs.step(action)
                ...
                additional_information = torch.tensor(values_list, dtype=torch.float32).to(self.config['device'])
                input = torch.cat([obs.view(-1, self.config["n_rollout_threads"], self.obs_size*self.num_agents), additional_information.view(-1, self.config["n_rollout_threads"], 4)], dim=-1)
            print('mpc cost is:', torch.sum(mpc_episode_costs))
    ...
    def cost_function(self, state):
        ...
        cost = x_velocity**2 + y_velocity**2 - (self._velocity_threshold-1.0)
        return cost
...
def train(args, cfg_train):
    ...
    parent_dir = os.path.dirname(current_file_dir)
    grandparent_dir = os.path.dirname(parent_dir)
    args.model_dir = grandparent_dir + '/logs/models_seed0'
...
if __name__ == '__main__':
    ...
    wandb.init(config=args,
            project='mampc',
            name='mampc_test_'+str(args.seed),
            job_type = "eval",
            reinit = True)
